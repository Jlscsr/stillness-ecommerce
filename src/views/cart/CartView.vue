<template>
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-32 pb-16">
    ...
    <div v-if="items.length === 0" class="text-center py-16 bg-cream border border-charcoal/10 rounded-sm">
      ...
      <p class="text-charcoal/70 mb-6">Looks like you haven't added any items to your cart yet.</p>
      <router-link
        to="/products"
    ...
    <div v-else class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      ...
      <div class="lg:col-span-2">
        <div class="bg-cream border border-charcoal/10 rounded-sm overflow-hidden">
          <div class="hidden sm:grid grid-cols-12 gap-4 p-4 border-b border-charcoal/10 bg-beige/10">
          ...
          <ul class="divide-y divide-charcoal/10">
            <li v-for="(item, index) in items" :key="`${item.product.id}-${item.variant || ''}-${index}`" class="p-4">
              <div class="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center">
                <!-- Product -->
                <div class="col-span-6 flex items-center">
                  <div class="h-20 w-20 flex-shrink-0 overflow-hidden rounded-sm bg-cream mr-4">
                    <img
                ...
                <!-- Price -->
                <div class="col-span-2 text-center">
                  <span class="text-sm text-charcoal">${{ item.product.price.toFixed(2) }}</span>
                </div>

                <!-- Quantity -->
                <div class="col-span-2">
                  <div class="flex items-center justify-center border border-charcoal/20 rounded-sm max-w-[120px] mx-auto">
                    <button
                ...
                <div class="col-span-2 flex items-center justify-between">
                  <span class="text-sm font-medium text-charcoal sm:hidden">Total:</span>
                  <div class="flex items-center">
                    ...
                    <button
                      ...
                      class="ml-4 text-charcoal/60 hover:text-terracotta transition-colors"
          ...
          <div class="flex-1 overflow-y-auto p-6">
            <div class="space-y-3 mb-6">
              ...
              <div class="border-t border-charcoal/10 pt-3 flex justify-between">
                ...
                <span class="font-medium text-charcoal">${{ total.toFixed(2) }}</span>
              </div>
            </div>

            <!-- Promo code -->
            <form @submit.prevent="handleApplyPromo" class="mb-6">
              <label for="promo-code" class="block text-sm font-medium text-charcoal mb-2">
                Promo Code
              </label>
              <div class="flex">
                <input
                ...
                <button
                  ...
                  :class="promoApplied ? 'bg-sage/50 text-cream cursor-not-allowed' : 'bg-sage text-cream hover:bg-sage/90'"
                  :disabled="promoApplied"
                >
                  Apply
                </button>
              </div>
              <p v-if="promoApplied" class="text-xs text-sage mt-1">Promo code "MINDFUL" applied successfully!</p>
            </form>
          </div>
...
// Summary height for sticky sidebar
...
const total = computed(() => subtotal.value + shipping + tax.value - discount.value);
